...
gives performance improvements. Despite "v6 plus" seeming to imply IPoE, as
best I can tell the router won't deal with that.
...
Most notably, with CGNAT, it's transparent to the router. The router simply
...
MAP-E is not transparent to the router. Instead, the router is told what IPv4 address it has (which is a real one!), and what ports on that IPv4 address it is allowed to use, and then it is expected to apply its own layer of NAT to ensure all outgoing traffic has a source IP of that address, and a source port of one of the allowed ports.

MAP-E also does one additional thing. It conveys to
the router an IPv6 address of a server which will accept encapsulated IPv4
traffic (encapsulated using an [IPv4 over IPv6
...
* ```rust
  ...
  packet, but HMARK ends up being much more compact, and seems like it could be
  nicer for a few cases as well, such as still giving a consistent source port
  in more cases.
